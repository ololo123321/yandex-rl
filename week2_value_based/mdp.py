...
def get_action_value(mdp, state_values, s, a, gamma):
    """Computes Q(s,a)"""
    p_sa = mdp._transition_probs[s].get(a, {})
    r_sa = mdp._rewards.get(s, {}).get(a, {})
    return sum(p_sa.get(s_next, 0) * (r_sa.get(s_next, 0) + gamma * state_values[s_next]) for s_next in state_values)


def get_new_state_value(mdp, state_values, s, gamma):
    """ Computes next V(s) as in formula above. Please do not change state_values in process. """
    if mdp.is_terminal(s): return 0
    possible_actions = mdp._transition_probs[s].keys()
    return max(get_action_value(mdp, state_values, s, a, gamma) for a in possible_actions)


def get_optimal_action(mdp, state_values, s, gamma=0.9):
    """ Finds optimal action using formula above. """
    if mdp.is_terminal(s): return None
    return sorted(mdp._transition_probs[s], key=lambda a: get_action_value(mdp, state_values, s, a, gamma))[-1]
...
def compute_vpi(mdp, policy, gamma):
    ...
    n_states = len(states)
    P = np.zeros((n_states, n_states))  # p(s'|s,a)
    R = np.zeros(n_states)  # E(R_t1|s,a)
    I = np.identity(n_states)
    for i, (s, a) in enumerate(policy.items()):
        transition_probs = mdp._transition_probs.get(s, {}).get(a, {})  # потому что в FrozenLake может выдать KeyError
        for s_next, p in transition_probs.items():
            r = mdp.get_reward(s, a, s_next)
            R[i] += r * p
            P[i, state_index[s_next]] = p
    A = I - gamma * P
...
def compute_new_policy(mdp, vpi, gamma):
    ...
    """
    states = mdp.get_all_states()
    actions = sorted(list(set(chain(*[mdp.get_possible_actions(s) for s in states]))))
    Q = np.zeros((len(states), len(actions)))
    for i, s in enumerate(states):
        for j, a in enumerate(actions):
            transition_probs = mdp._transition_probs.get(s, {}).get(a, {})
            for s_next, p in transition_probs.items():
                r = mdp.get_reward(s, a, s_next)
                Q[i, j] += p * (r + gamma * vpi[s_next])
    new_policy = {s: actions[i] for s, i in zip(states, np.argmax(Q, axis=1))}
    return new_policy
...
def policy_iteration(mdp, policy=None, gamma=0.9, num_iter=1000, min_difference=1e-5):
    ...
    # потому что в frozen lake есть состояния, в которых нет возможных действий (random.choice ругается)
    if not policy:
        policy = {}
        for s in mdp.get_all_states():
            actions = mdp.get_possible_actions(s)
            policy[s] = np.random.choice(actions) if actions else ()
...
class MDP:
    ...
    def get_all_states(self):
        ...
        return tuple(sorted(self._transition_probs.keys()))  # зачем tuple?
    ...
    for state_node in mdp._transition_probs:
        ...
        for posible_action in mdp.get_possible_actions(state_node):
            ...
            for posible_next_state in mdp.get_next_states(state_node, posible_action):
                probability = mdp.get_transition_prob( state_node, posible_action, posible_next_state)
